...
> English, no jargon. For the component/flow view with diagrams, see
> **[`HLD.md`](HLD.md)** (high-level design + the flow of every execution).
...
| 2026-09-21 | 2.4 | Documentation layer completed: [`EXPLAINER.md`](EXPLAINER.md) (plain-English guide) and **[`HLD.md`](HLD.md)** (high-level design + the execution flow of every endpoint, as Mermaid diagrams with an ASCII fallback). New `tests/test_docs.py` (45 tests) locks the docs to the code: fence/diagram structure and participant declaration order, relative-link resolution, documented endpoints/queues/task names/container caps/concurrency, and every quoted guardrail figure checked against `app/config.py`. All 7 Mermaid diagrams were validated with the real `mermaid.parse` grammar, and the ASCII topology is generated with column-exact geometry (asserted, not hand-aligned).
